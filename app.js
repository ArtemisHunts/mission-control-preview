...
function renderWorld() {
  els.roomLayer.innerHTML = Object.entries(ROOM_LAYOUT).map(([id, room]) => {
    const tasks = state.tasks.filter(task => task.room === id);
    const agents = state.agents.filter(agent => agent.location === id);
      ...
      <div class="room-card status-${roomStatus} ${selectedRoomId === id ? 'selected' : ''}" data-room-id="${id}" style="--x:${room.x}px; --y:${room.y}px; --w:${room.w}px; --h:${room.h}px; --tone:${room.tone}; --accent:${room.accent};">
        <div class="room-inner">
          ...
          <div class="room-stats">
            ...
            <span class="room-stat">${statusLabel(roomStatus)}</span>
          </div>
        </div>
      </div>
    `;
  ...
  const roomBuckets = Object.keys(ROOM_LAYOUT).reduce((acc, roomId) => {
    acc[roomId] = state.agents.filter(agent => agent.location === roomId);
    return acc;
  }, {});
  ...
  Object.entries(roomBuckets).forEach(([roomId, agents]) => {
    ...
    agents.forEach((agent, index) => {
      ...
      agentBits.push(`
        <div class="agent-label" style="--x:${point.x}px; --y:${point.y}px;">${escapeHtml(agent.name)}</div>
        <div class="agent-sprite agent-${escapeHtml(agent.status)} ${selectedAgentId === agent.id ? 'selected' : ''}" data-agent-id="${agent.id}" style="--x:${point.x}px; --y:${point.y}px;"></div>
      `);
    });
  });
  ...
  for (let i = 0; i < courierCount; i += 1) {
    const point = { x: 474 + i * 38, y: 298 + i * 10 };
    agentBits.push(`
      <div class="agent-label" style="--x:${point.x}px; --y:${point.y}px;">courier</div>
      <div class="agent-sprite agent-courier" style="--x:${point.x}px; --y:${point.y}px;"></div>
    `);
  }

  els.agentLayer.innerHTML = agentBits.join('');
}
...
function renderFocus() {
  ...
  if (room) {
    const roomTasks = state.tasks.filter(item => item.room === roomId);
    const roomAgents = state.agents.filter(item => item.location === roomId);
    els.focusTitle.textContent = room.label;
    els.focusCard.className = 'focus-card';
    els.focusCard.innerHTML = `
      <div class="focus-title-row">
        <span class="badge">${roomTasks.length} missions</span>
...
function renderTaskCard(task) {
  return `
    <div class="task-card ${selectedTaskId === task.id ? 'selected' : ''}" data-task-id="${task.id}">
      <div class="task-top">
        <span class="priority-pill priority-${task.priority}">P${task.priority}</span>
        <span class="badge">${escapeHtml(task.project || 'Mission Control')}</span>
      </div>
...
function roomSlot(room, index) {
  ...
  return {
    x: room.x + 42 + (index % cols) * 54,
    y: room.y + 84 + Math.floor(index / cols) * 42
  };
}
