...
function buildOffice() {
  ...
  buildNorthStarPressureGasket();
  buildOverviewCommandReadLines();
  buildFourDistrictExternalReadFrames();
  buildShellScaleSilhouettes();
  buildExteriorStationInfrastructure();
  buildLowerMechanicalCutaway();
  ...
  buildCinematicShellContrastPass();
...
function buildCutawayScaleDatumRibs() {
  const darkRib = mat(0x07080f, { roughness: 0.72, metalness: 0.54 });
  const graphite = mat(0x151d2a, { roughness: 0.58, metalness: 0.62 });
  const black = mat(0x010207, { roughness: 1.0, metalness: 0.0 });
  const amber = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.2, transparent: true, opacity: 0.2 });
  const cyan = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.18, transparent: true, opacity: 0.18 });
  const cutDust = mat(0x433548, { roughness: 0.98, metalness: 0.01 });

  // Pull the asteroid shell back into an architectural read: visible datum ribs show the thickness of the vertical cut.
  const sideRibs = [
    [22.7, 0.9, 8.0, 2.4],
    [22.95, 1.55, 6.2, 2.9],
    [23.15, 2.2, 4.0, 3.5],
    [23.0, 2.86, 1.72, 4.0],
    [22.74, 3.52, -0.72, 4.6],
    [22.45, 4.16, -3.14, 4.25],
    [22.14, 4.78, -5.7, 3.75],
    [21.76, 5.32, -8.4, 3.1],
    [21.2, 5.86, -11.0, 2.7],
    [20.55, 6.28, -13.4, 2.25]
  ];

  sideRibs.forEach(([xAbs, y, z, height], i) => {
    [-1, 1].forEach((side) => {
      const rib = box('cutaway scale datum vertical pressure rib', [0.11, height, 0.12], [side * xAbs, y + height * 0.38, z], i % 3 === 0 ? graphite : darkRib);
      rib.rotation.z = side * (0.05 + i * 0.008);
      rib.rotation.y = side * 0.08;

      const cap = box('cutaway scale datum rib amber cap mark', [0.13, 0.04, 0.54 + (i % 3) * 0.18], [side * (xAbs - 0.12), y + height * 0.76, z + 0.16], i % 2 ? cyan : amber);
      cap.rotation.z = rib.rotation.z;
      cap.rotation.y = rib.rotation.y;

      const cutShadow = box('cutaway scale datum rib contact shadow', [0.07, height * 0.72, 0.08], [side * (xAbs - 0.46), y + height * 0.32, z - 0.24], black);
      cutShadow.rotation.z = rib.rotation.z * 0.7;
      cutShadow.rotation.y = rib.rotation.y * 0.65;
    });
  });

  for (let i = 0; i < 16; i += 1) {
    const x = -21.0 + i * 2.8;
    const z = 7.25 - (i % 8) * 1.02;
    const hanger = box('cutaway roof survey rib visible at wide overview', [0.09, 1.85 + (i % 3) * 0.32, 0.09], [x, 7.62 - (i % 2) * 0.12, z], i % 2 ? darkRib : graphite);
    hanger.rotation.z = -0.08 + (i % 5) * 0.04;
    hanger.rotation.y = -0.18 + (i % 4) * 0.1;

    const lightTick = box('cutaway roof rib small practical tick', [0.36, 0.04, 0.04], [x + 0.32, 6.72 - (i % 2) * 0.16, z + 0.18], i % 3 === 0 ? amber : cyan);
    lightTick.rotation.y = hanger.rotation.y;
  }

  for (let i = 0; i < 15; i += 1) {
    const x = -20.0 + i * 2.86;
    const foot = box('cutaway sill footplate scale datum', [1.45 + (i % 4) * 0.24, 0.07, 0.18], [x, 0.18 + (i % 2) * 0.04, 9.05 + Math.sin(i) * 0.12], i % 2 ? graphite : darkRib);
    foot.rotation.y = 0.24 - (i % 5) * 0.09;

    const dust = box('cutaway sill crushed aggregate datum shadow', [1.0 + (i % 3) * 0.22, 0.045, 0.1], [x + 0.42, 0.34, 8.78 + Math.cos(i) * 0.12], cutDust);
    dust.rotation.y = foot.rotation.y * 0.75;
  }
  ...
  crossBraces.forEach(([x, y, z, rz], i) => {
    const brace = box('cutaway crown diagonal survey brace silhouette', [0.075, 2.8 + (i % 2) * 0.45, 0.075], [x, y, z], i % 2 ? graphite : darkRib);
    brace.rotation.z = rz;
    brace.rotation.y = -0.06 + i * 0.02;
  });
}

function buildDeepSideCavernVoids() {
  const voidMat = mat(0x000106, { roughness: 1.0, metalness: 0.0 });
  const wall = mat(0x07101d, { roughness: 0.78, metalness: 0.22 });
  const steel = mat(0x202c3e, { roughness: 0.46, metalness: 0.64 });
  const shadowSteel = mat(0x0b111d, { roughness: 0.64, metalness: 0.55 });
  const cyanDim = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.18, transparent: true, opacity: 0.17 });
  const amberDim = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.18, transparent: true, opacity: 0.16 });

  // Large side voids keep the user from feeling boxed inside the room: the base visibly continues left/right and backward.
  const voids = [
    [-21.2, 1.95, 1.9, 4.8, 2.7, COLORS.gold, 'build-side transfer cavern'],
    [21.2, 1.95, 1.9, 4.8, 2.7, COLORS.coral, 'review-side transfer cavern'],
    [-21.45, 2.45, -7.8, 5.4, 3.1, COLORS.violet, 'observatory-side signal cavern'],
    [21.45, 2.45, -7.8, 5.4, 3.1, COLORS.green, 'deploy-side logistics cavern'],
    [-18.5, 3.35, -16.2, 4.1, 2.55, COLORS.cyan, 'rear-left machinery cavern'],
    [18.5, 3.35, -16.2, 4.1, 2.55, COLORS.cyan, 'rear-right machinery cavern']
  ];

  voids.forEach(([x, y, z, depth, height, color, label], index) => {
    const side = x < 0 ? -1 : 1;
    const accent = mat(color, { emissive: color, emissiveIntensity: 0.18, transparent: true, opacity: 0.18 });
    const group = new THREE.Group();
    group.name = label;
    group.position.set(x, 0, z);
    root.add(group);

    box('deep side cavern black negative-space mouth', [0.18, height, depth], [0, y, 0], voidMat, group);
    box('deep side cavern inner wall plane', [0.12, height * 0.82, depth * 0.88], [side * -0.34, y, -0.08], wall, group);
    box('deep side cavern foreground pressure jamb', [0.18, height * 1.12, 0.16], [side * 0.08, y + 0.05, depth * 0.48], steel, group);
    box('deep side cavern rear pressure jamb', [0.16, height * 0.92, 0.14], [side * 0.02, y, -depth * 0.48], shadowSteel, group);
    box('deep side cavern service deck slab', [0.68, 0.08, depth * 0.78], [side * 0.18, y - height * 0.36, 0.02], shadowSteel, group);
    box('deep side cavern glowing rail perspective line', [0.05, 0.04, depth * 0.72], [side * 0.52, y - height * 0.18, 0.02], accent, group);
    box('deep side cavern back rim light', [0.045, height * 0.68, 0.06], [side * 0.44, y + 0.06, -depth * 0.43], index % 2 ? cyanDim : amberDim, group);

    for (let i = 0; i < 5; i += 1) {
      const zz = -depth * 0.36 + i * (depth * 0.18);
      box('deep side cavern receding bay divider', [0.055, height * 0.52, 0.045], [side * 0.34, y - height * 0.02, zz], i % 2 ? steel : shadowSteel, group);
      box('deep side cavern tiny maintenance window', [0.045, 0.14, 0.32], [side * 0.5, y + (i % 3) * 0.22, zz + 0.1], i % 2 ? accent : wall, group);
    }

    const haze = new THREE.Mesh(new THREE.PlaneGeometry(1.1, depth * 0.9), mat(color, { emissive: color, emissiveIntensity: 0.1, transparent: true, opacity: 0.025, side: THREE.DoubleSide }));
    haze.name = 'deep side cavern faint atmospheric slit';
    haze.position.set(x + side * 0.28, y, z);
    haze.rotation.y = side * Math.PI / 2;
    haze.rotation.z = -0.05 + index * 0.02;
    root.add(haze);
    animated.push((t) => { haze.material.opacity = 0.018 + Math.sin(t * 0.35 + index) * 0.006; });
  });

  const rearDrifts = [
    [-14.4, -22.8, COLORS.violet],
    [-7.2, -25.4, COLORS.gold],
    [0, -28.2, COLORS.cyan],
    [7.2, -25.4, COLORS.coral],
    [14.4, -22.8, COLORS.green]
  ];

  rearDrifts.forEach(([x, z, color], i) => {
    const shaft = box('deep side cavern distant vertical lift silhouette', [0.22, 3.8 - i * 0.22, 0.16], [x, 2.9, z], shadowSteel);
    shaft.rotation.z = x < 0 ? -0.04 : x > 0 ? 0.04 : 0;
    const beacon = box('deep side cavern distant lift beacon', [0.12, 0.055, 0.04], [x, 4.78 - i * 0.18, z + 0.16], mat(color, { emissive: color, emissiveIntensity: 0.3, transparent: true, opacity: 0.2 }));
    beacon.rotation.y = i * 0.08;
  });
}


function buildRearHorizonScaleMarkers() {
  const voidMat = mat(0x000105, { roughness: 1.0, metalness: 0.0 });
  const deckMat = mat(0x080f1a, { roughness: 0.66, metalness: 0.42 });
  const ribMat = mat(0x1c2738, { roughness: 0.48, metalness: 0.66 });
  const darkRib = mat(0x060a12, { roughness: 0.7, metalness: 0.52 });
  const cyanTiny = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.2, transparent: true, opacity: 0.16 });
  const amberTiny = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.18, transparent: true, opacity: 0.15 });
  const greenTiny = mat(COLORS.green, { emissive: COLORS.green, emissiveIntensity: 0.16, transparent: true, opacity: 0.14 });
  const coralTiny = mat(COLORS.coral, { emissive: COLORS.coral, emissiveIntensity: 0.16, transparent: true, opacity: 0.14 });

  // Far production horizons give the pulled-back view a real back-of-base read without adding interior detail.
  const horizons = [
    [-19.4, 31.5, 0.95, 4.9],
    [-23.8, 27.2, 1.35, 4.1],
    [-28.6, 21.8, 1.72, 3.4],
  ...
  horizons.forEach(([z, width, y, height], row) => {
    box('rear horizon black production void beyond cutaway', [width, height, 0.1], [0, y + height * 0.5, z], voidMat);
    box('rear horizon suspended deck silhouette', [width * 0.82, 0.07, 0.5], [0, y, z + 0.38], row % 2 ? darkRib : deckMat);
    box('rear horizon upper service line', [width * 0.74, 0.055, 0.07], [0, y + height * 0.82, z + 0.18], ribMat);
    box('rear horizon mid pipe run', [width * 0.58, 0.045, 0.06], [0, y + height * 0.48, z + 0.22], darkRib);

    [-1, 1].forEach((side) => {
      box('rear horizon side lift tower silhouette', [0.18, height * 0.78, 0.12], [side * width * 0.43, y + height * 0.42, z + 0.25], ribMat);
      box('rear horizon inner service post silhouette', [0.1, height * 0.58, 0.08], [side * width * 0.28, y + height * 0.36, z + 0.28], darkRib);
      const diagonalA = box('rear horizon long diagonal truss silhouette', [0.06, height * 0.58, 0.06], [side * width * 0.34, y + height * 0.42, z + 0.3], darkRib);
      diagonalA.rotation.z = side * (0.22 + row * 0.03);
      const diagonalB = box('rear horizon opposing diagonal truss silhouette', [0.055, height * 0.44, 0.055], [side * width * 0.18, y + height * 0.32, z + 0.31], ribMat);
      diagonalB.rotation.z = side * (-0.2 + row * 0.02);
    });
  });

  for (let row = 0; row < 6; row += 1) {
    const z = -18.8 - row * 4.15;
    const width = 30 - row * 3.9;
    const count = 12 - row;
    const yBase = 1.55 + row * 0.3;
    for (let i = 0; i < count; i += 1) {
      const x = -width * 0.44 + i * (width * 0.88 / Math.max(1, count - 1));
      const y = yBase + (i % 5) * 0.22;
      const material = (i + row) % 4 === 0 ? cyanTiny : (i + row) % 4 === 1 ? amberTiny : (i + row) % 4 === 2 ? greenTiny : coralTiny;
      const light = box('rear horizon tiny plant window scale marker', [0.12, 0.04, 0.032], [x, y, z], material);
      light.rotation.y = (i - count * 0.5) * 0.01;
    }
  }

  const conveyorRuns = [
    [-13.5, -17.4, -7.5, -29.2, COLORS.violet],
    [-4.8, -18.8, -2.0, -34.0, COLORS.gold],
    [4.8, -18.8, 2.0, -34.0, COLORS.coral],
    [13.5, -17.4, 7.5, -29.2, COLORS.green]
  ];

  conveyorRuns.forEach(([x1, z1, x2, z2, color], index) => {
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(x1, 1.35, z1),
      new THREE.Vector3((x1 + x2) * 0.5, 1.9 + index * 0.08, (z1 + z2) * 0.5),
      new THREE.Vector3(x2, 2.25 + index * 0.08, z2)
    ]);
    const tube = new THREE.Mesh(
      new THREE.TubeGeometry(curve, 32, 0.022, 8, false),
      mat(color, { emissive: color, emissiveIntensity: 0.16, transparent: true, opacity: 0.14 })
    );
    tube.name = 'rear horizon distant conveyor path into production base';
    root.add(tube);

    for (let i = 0; i < 4; i += 1) {
      const p = curve.getPoint((i + 1) / 5);
      const support = box('rear horizon conveyor hanger scale post', [0.045, 0.78 + i * 0.08, 0.045], [p.x, p.y - 0.38, p.z], i % 2 ? ribMat : darkRib);
      support.rotation.z = x1 < 0 ? -0.04 : 0.04;
    }
  });

  const farShips = [
    [-6.4, 3.3, -36.8, 1.0, cyanTiny],
    [6.4, 3.15, -36.2, 0.88, amberTiny],
    [0, 3.75, -42.0, 0.72, cyanTiny]
  ];

  farShips.forEach(([x, y, z, scale, material], i) => {
    const ship = new THREE.Group();
    ship.name = 'rear horizon tiny docked ship silhouette for scale';
    ship.position.set(x, y, z);
    ship.scale.setScalar(scale);
    root.add(ship);
    box('rear horizon ship spine', [1.1, 0.08, 0.16], [0, 0, 0], darkRib, ship);
    box('rear horizon ship nose wedge', [0.26, 0.11, 0.18], [0.68, 0, 0], ribMat, ship);
    box('rear horizon ship left wing', [0.38, 0.045, 0.42], [-0.18, -0.01, 0.28], deckMat, ship);
    box('rear horizon ship right wing', [0.38, 0.045, 0.42], [-0.18, -0.01, -0.28], deckMat, ship);
    box('rear horizon ship running light', [0.07, 0.035, 0.035], [0.78, 0.04, 0], material, ship);
    ship.rotation.y = -0.18 + i * 0.16;
  });

  const vaporMat = (color, opacity) => mat(color, { emissive: color, emissiveIntensity: 0.08, transparent: true, opacity, side: THREE.DoubleSide });
  [
    [-10.5, 3.1, -25.8, COLORS.violet, 0.018],
    [10.5, 3.1, -25.8, COLORS.green, 0.018],
    [-4.0, 3.8, -33.4, COLORS.gold, 0.014],
    [4.0, 3.8, -33.4, COLORS.coral, 0.014],
    [0, 4.2, -40.0, COLORS.cyan, 0.012]
  ].forEach(([x, y, z, color, opacity], i) => {
    const plume = new THREE.Mesh(new THREE.PlaneGeometry(1.8 - i * 0.12, 6.0 - i * 0.5), vaporMat(color, opacity));
    plume.name = 'rear horizon faint vertical production haze';
    plume.position.set(x, y, z);
    plume.rotation.x = -0.02;
    plume.rotation.z = x < 0 ? 0.04 : x > 0 ? -0.04 : 0;
    root.add(plume);
    animated.push((t) => { plume.material.opacity = opacity + Math.sin(t * 0.27 + i) * 0.004; });
  });
}


function buildForegroundApertureScaleShadow() {
  const shadow = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.72 });
  const graphite = mat(0x111827, { roughness: 0.58, metalness: 0.52 });
  const cutFace = mat(0x342a36, { roughness: 0.96, metalness: 0.02 });
  const amber = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.16, transparent: true, opacity: 0.16 });
  const cyan = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.15, transparent: true, opacity: 0.15 });

  // Foreground parallax shadow gives the viewer a safe exterior ledge instead of the feeling of standing inside the room.
  const slabs = [
    [-20.6, 0.12, 10.58, 6.8, -0.04],
    [-11.8, 0.04, 10.92, 8.4, 0.03],
    [-2.8, 0.0, 10.72, 6.2, -0.02],
    [5.2, 0.02, 10.96, 7.6, 0.025],
    [15.2, 0.1, 10.62, 9.2, -0.035]
  ];

  slabs.forEach(([x, y, z, width, rz], i) => {
    const slab = box('foreground aperture massive shadowed viewing ledge', [width, 0.16, 0.52], [x, y, z], i % 2 ? graphite : shadow);
    slab.rotation.z = rz;
    slab.rotation.y = -0.08 + i * 0.04;
    const face = box('foreground aperture ledge exposed sliced face', [width * 0.86, 0.08, 0.1], [x + 0.2, y + 0.18, z - 0.3], cutFace);
    face.rotation.z = rz * 0.8;
    face.rotation.y = slab.rotation.y;
  });

  for (let i = 0; i < 18; i += 1) {
    const x = -22.0 + i * 2.58;
    const crack = box('foreground aperture ledge irregular black crack', [0.85 + (i % 4) * 0.35, 0.045, 0.06], [x, 0.36 + Math.sin(i) * 0.04, 10.18 + Math.cos(i * 0.7) * 0.16], shadow);
    crack.rotation.y = 0.22 - (i % 6) * 0.08;
    const tick = box('foreground aperture ledge tiny scale glint', [0.09, 0.035, 0.18], [x + 0.46, 0.48, 9.9], i % 3 === 0 ? cyan : amber);
    tick.rotation.y = crack.rotation.y;
  }

  for (let i = 0; i < 10; i += 1) {
    const x = -19.5 + i * 4.3;
    const post = box('foreground aperture exterior survey post silhouette', [0.07, 0.82 + (i % 2) * 0.22, 0.07], [x, 0.86, 9.58], i % 2 ? graphite : shadow);
    post.rotation.z = -0.06 + i * 0.014;
    box('foreground aperture survey post dim marker', [0.18, 0.045, 0.045], [x, 1.32 + (i % 2) * 0.11, 9.56], i % 2 ? cyan : amber);
  }

  [-16.8, -8.4, 0, 8.4, 16.8].forEach((x, i) => {
    const cutMark = box('foreground aperture ledge broad cross-section datum', [2.8, 0.04, 0.055], [x, 0.62 + (i % 2) * 0.04, 9.36], i % 2 ? cutFace : graphite);
    cutMark.rotation.y = -0.18 + i * 0.09;
    box('foreground aperture ledge datum shadow undercut', [1.8, 0.035, 0.045], [x + 0.36, 0.5, 9.48], shadow);
  });

  const centerSightline = box('foreground aperture centerline depth notch', [0.08, 0.05, 0.42], [0, 0.76, 9.18], cyan);
  centerSightline.rotation.y = 0.12;
}

function buildDistrictSilhouetteLightWells() {
  const blackGlass = mat(0x030711, { roughness: 0.18, metalness: 0.22, transparent: true, opacity: 0.58 });
  const floorShadow = mat(0x010207, { roughness: 0.95, metalness: 0.0, transparent: true, opacity: 0.6 });
  const graphite = mat(0x121a29, { roughness: 0.5, metalness: 0.54 });

  // Wide camera fix: each district gets a subtle backlight well so all four remain readable inside the larger aperture.
  const wells = [
    ['build', COLORS.gold, -12.75, 2.08, -0.25, 0.05, 6.4, 3.2],
    ['review', COLORS.coral, 12.75, 2.08, -0.45, -0.05, 6.4, 3.2],
    ['observatory', COLORS.violet, -12.85, 2.18, -10.65, 0.035, 6.1, 3.35],
    ['deploy', COLORS.green, 12.85, 2.18, -10.65, -0.035, 6.1, 3.35]
  ];

  wells.forEach(([id, color, x, y, z, rz, width, height], index) => {
    const glow = mat(color, { emissive: color, emissiveIntensity: 0.18, transparent: true, opacity: 0.075, side: THREE.DoubleSide, roughness: 0.1 });
    const edge = mat(color, { emissive: color, emissiveIntensity: 0.36, transparent: true, opacity: 0.26 });
    const group = new THREE.Group();
    group.name = `${id} wide overview silhouette light well`;
    group.position.set(x, 0, z);
    group.rotation.z = rz;
    root.add(group);

    const back = new THREE.Mesh(new THREE.PlaneGeometry(width, height), glow);
    back.name = `${id} district wide backlight plane`;
    back.position.set(0, y, -0.72);
    back.rotation.x = -0.14;
    group.add(back);
    animated.push((t) => { back.material.opacity = 0.06 + Math.sin(t * 0.42 + index) * 0.01; });

    box('district silhouette light well top rim', [width * 0.92, 0.05, 0.06], [0, y + height * 0.42, -0.62], edge, group);
    box('district silhouette light well bottom rim', [width * 0.76, 0.035, 0.055], [0, y - height * 0.38, -0.58], graphite, group);
    box('district silhouette light well left side rim', [0.06, height * 0.76, 0.055], [-width * 0.48, y, -0.6], edge, group);
    box('district silhouette light well right side rim', [0.06, height * 0.76, 0.055], [width * 0.48, y, -0.6], edge, group);

    const shadow = new THREE.Mesh(new THREE.PlaneGeometry(width * 1.08, 2.2), floorShadow);
    shadow.name = `${id} district foreground shadow moat for silhouette contrast`;
    shadow.position.set(0, 0.07, 0.64);
    shadow.rotation.x = -Math.PI / 2;
    group.add(shadow);

    for (let i = 0; i < 4; i += 1) {
      const px = -width * 0.32 + i * (width * 0.22);
      const column = box('district light well receding aperture mullion', [0.045, height * 0.55, 0.04], [px, y + 0.02, -0.55 - i * 0.04], i % 2 ? edge : graphite, group);
      column.rotation.z = -0.035 + i * 0.02;
    }

    for (let i = 0; i < 5; i += 1) {
      const pip = box('district light well tiny scale pip visible in overview', [0.11, 0.032, 0.035], [-width * 0.36 + i * width * 0.18, y + height * 0.27, -0.5], edge, group);
      pip.rotation.y = -0.1 + i * 0.05;
    }

    const glass = new THREE.Mesh(new THREE.PlaneGeometry(width * 0.72, height * 0.45), blackGlass);
    glass.name = `${id} district dark glass silhouette patch`;
    glass.position.set(0, y - 0.1, -0.5);
    glass.rotation.x = -0.1;
    group.add(glass);
  });
}

function buildOverviewAtmosphericGradient() {
  const rearCool = addLight('directional', 0x88bdff, 0.42, [0, 9.0, -34.0]);
  rearCool.name = 'overview atmospheric rear blue rim gradient';
  rearCool.target.position.set(0, 2.2, -5.8);
  scene.add(rearCool.target);

  const crownFill = addLight('directional', 0xffd28a, 0.22, [0, 12.0, 8.0]);
  crownFill.name = 'overview warm crown grazing fill';
  crownFill.target.position.set(0, 1.4, -3.0);
  scene.add(crownFill.target);

  addLight('point', 0x78b6ff, 1.55, [-24.0, 5.8, -8.0], 25.0);
  addLight('point', 0x78b6ff, 1.55, [24.0, 5.8, -8.0], 25.0);
  addLight('point', COLORS.amber, 1.1, [-18.0, 0.9, 8.8], 19.0);
  addLight('point', COLORS.amber, 1.1, [18.0, 0.9, 8.8], 19.0);

  const veilMat = (color, opacity) => mat(color, {
    emissive: color,
  ...
  const veils = [
    [0, 4.2, -17.8, 34.0, 4.0, 0x5e9de8, 0.026, -0.05],
    [0, 4.55, -25.5, 25.0, 3.5, 0x5e9de8, 0.018, -0.035],
    [0, 4.9, -33.0, 16.0, 2.8, 0x5e9de8, 0.014, -0.02],
    [-12.4, 2.4, 4.8, 12.0, 1.9, COLORS.amber, 0.012, -0.16],
    [12.4, 2.4, 4.8, 12.0, 1.9, COLORS.amber, 0.012, 0.16],
    [0, 1.35, 9.2, 36.0, 1.2, 0x090d16, 0.34, 0]
  ];

  veils.forEach(([x, y, z, w, h, color, opacity, rx], i) => {
    const veil = new THREE.Mesh(new THREE.PlaneGeometry(w, h), veilMat(color, opacity));
    veil.name = 'overview atmospheric layered readability gradient';
    veil.position.set(x, y, z);
    veil.rotation.x = rx;
    veil.rotation.z = i % 2 ? 0.015 : -0.015;
    root.add(veil);
    animated.push((t) => { veil.material.opacity = opacity + Math.sin(t * 0.2 + i) * (opacity < 0.05 ? 0.004 : 0.018); });
  });

  const apertureHighlights = [
    [-23.0, 2.4, 7.1, 0.045, 5.2, COLORS.cyan, -0.07],
    [23.0, 2.4, 7.1, 0.045, 5.2, COLORS.cyan, 0.07],
    [-22.6, 4.8, -5.1, 0.045, 4.8, COLORS.amber, -0.045],
    [22.6, 4.8, -5.1, 0.045, 4.8, COLORS.amber, 0.045],
    [-14.0, 8.05, 4.8, 8.8, 0.045, COLORS.cyan, -0.02],
    [14.0, 8.05, 4.8, 8.8, 0.045, COLORS.cyan, 0.02],
    [0, 7.72, -8.4, 18.0, 0.04, COLORS.amber, 0]
  ];

  apertureHighlights.forEach(([x, y, z, w, h, color, rz], i) => {
    const size = w < 0.1 ? [0.055, h, 0.05] : [w, h, 0.05];
    const edge = box('overview aperture readability edge gradient catchlight', size, [x, y, z], veilMat(color, i % 2 ? 0.16 : 0.13));
    edge.rotation.z = rz;
  });
  ...
  bottomOcclusion.forEach(([x, y, z, w], i) => {
    const shade = new THREE.Mesh(new THREE.PlaneGeometry(w, 1.6), veilMat(0x000106, 0.26));
    shade.name = 'overview foreground sill shadow gradient';
...
function buildNorthStarPressureGasket() {
  const graphite = mat(0x121a27, { roughness: 0.46, metalness: 0.72 });
  const darkMetal = mat(0x050812, { roughness: 0.52, metalness: 0.66 });
  const blackGlass = mat(0x020611, { roughness: 0.12, metalness: 0.35, transparent: true, opacity: 0.58, emissive: 0x061626, emissiveIntensity: 0.1 });
  const coldEdge = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.34, transparent: true, opacity: 0.24 });
  const warmEdge = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.28, transparent: true, opacity: 0.22 });
  const gasketShadow = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.82 });

  // Reference push: rock alone reads primitive; the north-star boards show engineered pressure envelopes embedded into the asteroid.
  // This gasket is the explicit metal-to-rock transition around the full default cutaway.
  const sideFrames = [
    [-19.9, 3.45, 5.55, 0.18, 6.7, 5.3, -0.05],
    [19.9, 3.45, 5.55, 0.18, 6.7, 5.3, 0.05],
    [-20.5, 3.78, -2.5, 0.16, 6.1, 6.4, 0.035],
    [20.5, 3.78, -2.5, 0.16, 6.1, 6.4, -0.035],
    [-19.0, 4.18, -10.8, 0.14, 5.0, 4.9, -0.025],
    [19.0, 4.18, -10.8, 0.14, 5.0, 4.9, 0.025]
  ];

  sideFrames.forEach(([x, y, z, w, h, d, rz], i) => {
    const spine = box('north-star pressure gasket vertical shell spine', [w, h, d], [x, y, z], i % 2 ? graphite : darkMetal);
    spine.rotation.z = rz;
    spine.rotation.y = x < 0 ? 0.055 : -0.055;
    const inner = box('north-star pressure gasket inner black-glass reveal', [0.06, h * 0.82, d * 0.84], [x * 0.985, y, z + 0.08], blackGlass);
    inner.rotation.z = rz * 0.8;
    inner.rotation.y = spine.rotation.y;

    for (let j = 0; j < 5; j += 1) {
      const yy = y - h * 0.36 + j * (h * 0.18);
      const zz = z - d * 0.36 + (j % 2) * d * 0.18;
      const tick = box('north-star pressure gasket small status bolt', [0.075, 0.038, 0.24], [x * 0.975, yy, zz], (i + j) % 2 ? coldEdge : warmEdge);
      tick.rotation.z = rz;
      tick.rotation.y = spine.rotation.y;
    }
  });

  const crownSegments = [
    [-18.0, 7.62, 5.0, 7.2, -0.04],
    [-9.0, 7.82, 2.2, 8.4, 0.03],
    [0, 7.95, -1.0, 8.8, -0.015],
    [9.0, 7.82, 2.2, 8.4, -0.03],
    [18.0, 7.62, 5.0, 7.2, 0.04]
  ];

  crownSegments.forEach(([x, y, z, width, rz], i) => {
    const cap = box('north-star pressure gasket crown pressure lintel', [width, 0.2, 0.46], [x, y, z], i % 2 ? graphite : darkMetal);
    cap.rotation.z = rz;
    cap.rotation.y = -0.08 + i * 0.04;
    box('north-star pressure gasket crown cold edge line', [width * 0.82, 0.035, 0.055], [x, y - 0.18, z - 0.2], i % 2 ? warmEdge : coldEdge).rotation.y = cap.rotation.y;
    box('north-star pressure gasket crown shadow pocket', [width * 0.72, 0.06, 0.08], [x + 0.2, y - 0.36, z + 0.28], gasketShadow).rotation.y = cap.rotation.y;
  });

  const sillSegments = [
    [-18.0, 0.42, 8.96, 7.6, 0.035],
    [-8.6, 0.34, 9.22, 8.1, -0.025],
    [0, 0.3, 9.02, 6.9, 0.01],
    [8.6, 0.34, 9.22, 8.1, 0.025],
    [18.0, 0.42, 8.96, 7.6, -0.035]
  ];

  sillSegments.forEach(([x, y, z, width, rz], i) => {
    const sill = box('north-star pressure gasket foreground sill armor plate', [width, 0.16, 0.54], [x, y, z], i % 2 ? darkMetal : graphite);
    sill.rotation.z = rz;
    sill.rotation.y = 0.08 - i * 0.04;
    box('north-star pressure gasket sill amber service line', [width * 0.72, 0.035, 0.055], [x, y + 0.16, z - 0.28], i % 2 ? coldEdge : warmEdge).rotation.y = sill.rotation.y;
  });

  for (let i = 0; i < 28; i += 1) {
    const x = -20.7 + i * 1.54;
    const y = i % 2 ? 7.4 : 0.78;
    const z = i % 2 ? 4.6 - (i % 7) * 0.9 : 8.5 + Math.sin(i) * 0.18;
    const bolt = box('north-star pressure gasket perimeter clamp block', [0.28, 0.08, 0.16], [x, y, z], i % 3 ? graphite : darkMetal);
    bolt.rotation.y = -0.18 + (i % 6) * 0.07;
  }

  [-16.4, -8.2, 0, 8.2, 16.4].forEach((x, i) => {
    const verticalSeal = box('north-star pressure gasket hanging seal strap', [0.075, 1.65 + (i % 2) * 0.28, 0.075], [x, 6.78, 5.2 - (i % 3) * 1.3], i % 2 ? darkMetal : graphite);
    verticalSeal.rotation.z = -0.03 + i * 0.015;
    box('north-star pressure gasket strap tiny inspection lamp', [0.16, 0.05, 0.05], [x, 5.86 - (i % 2) * 0.14, 5.2 - (i % 3) * 1.3], i % 2 ? coldEdge : warmEdge);
  });
}

function buildOverviewCommandReadLines() {
  const cyanMat = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.42, transparent: true, opacity: 0.2 });
  const amberMat = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.36, transparent: true, opacity: 0.18 });
  const graphite = mat(0x101827, { roughness: 0.52, metalness: 0.58 });
  const shadow = mat(0x010207, { roughness: 0.95, metalness: 0.0, transparent: true, opacity: 0.5 });
  const laneMat = (color, opacity) => mat(color, { emissive: color, emissiveIntensity: 0.28, transparent: true, opacity, side: THREE.DoubleSide, roughness: 0.18 });

  // The reference boards have one clean command hierarchy. These wide-view read lines pull the eye back to the holo-table.
  const rings = [
    [2.8, 0.028, COLORS.cyan, 0.18],
    [4.4, 0.022, COLORS.amber, 0.14],
    [6.2, 0.018, COLORS.cyan, 0.1]
  ];

  rings.forEach(([radius, tube, color, opacity], i) => {
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(radius, tube, 8, 128),
      mat(color, { emissive: color, emissiveIntensity: 0.26 - i * 0.04, transparent: true, opacity, roughness: 0.2, metalness: 0.1 })
    );
    ring.name = 'north-star overview command hierarchy floor ring';
    ring.position.set(0, 0.075 + i * 0.004, 0.55);
    ring.rotation.x = Math.PI / 2;
    root.add(ring);
  });

  const roomLinks = [
    ['build', COLORS.gold, -12.75, -0.25, -5.3, 1.6],
    ['review', COLORS.coral, 12.75, -0.45, 5.3, 1.6],
    ['observatory', COLORS.violet, -12.85, -10.65, -5.4, -4.4],
    ['deploy', COLORS.green, 12.85, -10.65, 5.4, -4.4]
  ];

  roomLinks.forEach(([id, color, x, z, midX, midZ], i) => {
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(0, 0.13, 0.55),
      new THREE.Vector3(midX, 0.14, midZ),
      new THREE.Vector3(x * 0.82, 0.15, z * 0.92)
    ]);
    const tube = new THREE.Mesh(new THREE.TubeGeometry(curve, 40, 0.018, 8, false), laneMat(color, 0.24));
    tube.name = `north-star ${id} district readable command lane`;
    root.add(tube);

    for (let j = 0; j < 4; j += 1) {
      const p = curve.getPoint((j + 1) / 5);
      const marker = box('north-star command lane tiny routing marker', [0.12, 0.035, 0.12], [p.x, p.y + 0.04, p.z], i % 2 ? cyanMat : amberMat);
      marker.rotation.y = j * 0.55;
    }
  });

  const centralShadow = new THREE.Mesh(new THREE.PlaneGeometry(10.8, 7.4), shadow);
  centralShadow.name = 'north-star command hierarchy surrounding shadow moat';
  centralShadow.position.set(0, 0.052, -0.6);
  centralShadow.rotation.x = -Math.PI / 2;
  root.add(centralShadow);

  const sightlineBrackets = [
    [-5.2, 0.18, 2.75, 2.4, 0.12],
    [5.2, 0.18, 2.75, 2.4, -0.12],
    [-5.7, 0.18, -4.25, 2.7, -0.08],
    [5.7, 0.18, -4.25, 2.7, 0.08]
  ];

  sightlineBrackets.forEach(([x, y, z, length, rz], i) => {
    const rail = box('north-star overview command sightline bracket rail', [length, 0.055, 0.08], [x, y, z], i % 2 ? graphite : darkenMat(graphite));
    rail.rotation.y = rz;
    box('north-star overview command sightline bracket light', [length * 0.72, 0.032, 0.045], [x, y + 0.06, z - 0.08], i % 2 ? cyanMat : amberMat).rotation.y = rz;
  });
}

function darkenMat(baseMaterial) {
  return mat(0x070b13, { roughness: baseMaterial.roughness ?? 0.6, metalness: baseMaterial.metalness ?? 0.5 });
}


function buildFourDistrictExternalReadFrames() {
  const graphite = mat(0x121a28, { roughness: 0.5, metalness: 0.62 });
  const dark = mat(0x050812, { roughness: 0.62, metalness: 0.46 });
  const glass = mat(0x030812, { roughness: 0.16, metalness: 0.2, transparent: true, opacity: 0.46, emissive: 0x07162a, emissiveIntensity: 0.08 });
  const shadow = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.52 });
  const accentMat = (color, opacity = 0.25) => mat(color, { emissive: color, emissiveIntensity: 0.34, transparent: true, opacity, roughness: 0.18, metalness: 0.05 });

  // Wide overview guardrail: every district gets an exterior architectural frame, not more interior clutter.
  // This keeps the four-station read aligned to the reference boards' clean operational hierarchy.
  const frames = [
    ['build', 'BUILD', COLORS.gold, -12.75, 1.95, -0.25, -0.035, -1],
    ['review', 'REVIEW', COLORS.coral, 12.75, 1.95, -0.45, 0.035, 1],
    ['observatory', 'SIGNAL', COLORS.violet, -12.85, 2.05, -10.65, -0.025, -1],
    ['deploy', 'DEPLOY', COLORS.green, 12.85, 2.05, -10.65, 0.025, 1]
  ];

  frames.forEach(([id, label, color, x, y, z, rz, side], index) => {
    const group = new THREE.Group();
    group.name = `north-star ${id} exterior district read frame`;
    group.position.set(x, 0, z);
    group.rotation.z = rz;
    root.add(group);

    const accent = accentMat(color, 0.26);
    const lowAccent = accentMat(color, 0.12);
    const wash = accentMat(color, 0.055);

    const backPlane = new THREE.Mesh(new THREE.PlaneGeometry(6.8, 3.6), wash);
    backPlane.name = `north-star ${id} district exterior silhouette wash plane`;
    backPlane.position.set(0, y, -1.05);
    backPlane.rotation.x = -0.12;
    group.add(backPlane);
    animated.push((t) => { backPlane.material.opacity = 0.046 + Math.sin(t * 0.36 + index) * 0.008; });

    box('north-star district exterior frame top pressure rail', [6.3, 0.08, 0.09], [0, y + 1.62, -0.92], index % 2 ? graphite : dark, group);
    box('north-star district exterior frame lower pressure rail', [5.5, 0.06, 0.08], [0, y - 1.42, -0.82], dark, group);
    box('north-star district exterior frame left jamb', [0.1, 2.84, 0.08], [-3.25, y + 0.02, -0.86], graphite, group);
    box('north-star district exterior frame right jamb', [0.1, 2.84, 0.08], [3.25, y + 0.02, -0.86], graphite, group);
    box('north-star district exterior frame accent header', [5.8, 0.035, 0.055], [0, y + 1.43, -0.75], accent, group);
    box('north-star district exterior frame accent side tick left', [0.045, 1.8, 0.05], [-3.02, y, -0.74], lowAccent, group);
    box('north-star district exterior frame accent side tick right', [0.045, 1.8, 0.05], [3.02, y, -0.74], lowAccent, group);

    for (let i = 0; i < 4; i += 1) {
      const px = -2.2 + i * 1.45;
      const mullion = box('north-star district exterior frame vertical mullion', [0.045, 2.05, 0.045], [px, y + 0.03, -0.72 - i * 0.035], i % 2 ? dark : graphite, group);
      mullion.rotation.z = -0.035 + i * 0.02;
      box('north-star district exterior frame tiny status slot', [0.24, 0.035, 0.035], [px + 0.28, y + 1.12 - (i % 2) * 0.2, -0.68], i % 2 ? accent : lowAccent, group);
    }

    const glassPatch = new THREE.Mesh(new THREE.PlaneGeometry(4.4, 1.72), glass);
    glassPatch.name = `north-star ${id} district readable dark-glass mass`;
    glassPatch.position.set(0, y - 0.08, -0.65);
    glassPatch.rotation.x = -0.08;
    group.add(glassPatch);

    const sideRail = box('north-star district exterior perspective rail to shell', [0.08, 0.06, 3.3], [side * 3.75, y - 0.9, -1.22], accent, group);
    sideRail.rotation.y = side * 0.18;

    const shadowMoat = new THREE.Mesh(new THREE.PlaneGeometry(6.9, 2.2), shadow);
    shadowMoat.name = `north-star ${id} district lower contrast moat`;
    shadowMoat.position.set(0, 0.08, 0.52);
    shadowMoat.rotation.x = -Math.PI / 2;
    group.add(shadowMoat);

    const labelSprite = makeTextSprite(label, `#${color.toString(16).padStart(6, '0')}`, 44);
    labelSprite.name = `north-star ${id} wide overview district label`;
    labelSprite.position.set(0, y + 1.82, -0.62);
    labelSprite.scale.set(1.55, 0.34, 1);
    labelSprite.material.opacity = 0.32;
    group.add(labelSprite);
  });

  // Fine scale lines connect the four frames back into the central command hierarchy.
  frames.forEach(([id, , color, x, , z], index) => {
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(x * 0.9, 1.34, z - 0.8),
      new THREE.Vector3(x * 0.52, 1.62 + index * 0.05, (z - 2.6) * 0.55),
      new THREE.Vector3(0, 1.55, -2.2)
    ]);
    const tube = new THREE.Mesh(new THREE.TubeGeometry(curve, 28, 0.016, 8, false), accentMat(color, 0.16));
    tube.name = `north-star ${id} frame-to-command sightline`;
    root.add(tube);
  });
}


function buildShellScaleSilhouettes() {
  const suit = mat(0x0b1018, { roughness: 0.62, metalness: 0.18 });
  const visor = mat(0x02060c, { roughness: 0.12, metalness: 0.32, emissive: 0x07182a, emissiveIntensity: 0.12 });
  const cyan = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.26, transparent: true, opacity: 0.22 });
  const amber = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.22, transparent: true, opacity: 0.2 });
  const shadow = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.45 });

  // Tiny silhouettes at the shell perimeter provide human scale like the north-star boards, without starting a prop/detail pass.
  const silhouettes = [
    [-17.8, 1.05, 6.2, -0.12, COLORS.cyan],
    [-14.6, 1.28, 4.6, -0.08, COLORS.amber],
    [14.6, 1.28, 4.6, 0.08, COLORS.amber],
    [17.8, 1.05, 6.2, 0.12, COLORS.cyan],
    [-12.8, 2.22, -13.8, -0.04, COLORS.violet],
    [12.8, 2.22, -13.8, 0.04, COLORS.green],
    [-5.6, 1.72, -18.2, -0.02, COLORS.gold],
    [5.6, 1.72, -18.2, 0.02, COLORS.coral]
  ];

  silhouettes.forEach(([x, y, z, rz, color], index) => {
    const group = new THREE.Group();
    group.name = 'north-star tiny shell crew silhouette scale reference';
    group.position.set(x, y, z);
    group.rotation.z = rz;
    root.add(group);

    box('tiny shell crew boot shadow', [0.34, 0.025, 0.22], [0, 0.015, 0.03], shadow, group);
    box('tiny shell crew torso silhouette', [0.16, 0.34, 0.09], [0, 0.28, 0], suit, group);
    box('tiny shell crew helmet silhouette', [0.18, 0.15, 0.12], [0, 0.56, 0], suit, group);
    box('tiny shell crew dark visor slit', [0.12, 0.04, 0.035], [0, 0.58, 0.07], visor, group);
    box('tiny shell crew left leg silhouette', [0.055, 0.22, 0.055], [-0.055, 0.08, 0], suit, group);
    box('tiny shell crew right leg silhouette', [0.055, 0.22, 0.055], [0.055, 0.08, 0], suit, group);
    box('tiny shell crew status glint', [0.04, 0.035, 0.025], [0.075, 0.36, 0.06], color === COLORS.cyan || index % 2 ? cyan : amber, group);
    group.scale.setScalar(index > 3 ? 0.78 : 0.92);
  });

  [-18.4, -9.2, 0, 9.2, 18.4].forEach((x, i) => {
    const tick = box('north-star shell crew scale railing tick', [0.08, 0.32 + (i % 2) * 0.08, 0.055], [x, 1.02 + (i % 2) * 0.06, 7.18], i % 2 ? cyan : amber);
    tick.rotation.z = -0.035 + i * 0.018;
    box('north-star shell crew scale railing shadow', [0.18, 0.035, 0.055], [x + 0.1, 0.86, 7.22], shadow);
  });

  [-13.6, 13.6].forEach((x, sideIndex) => {
    const watchPlatform = box('north-star shell tiny observation platform silhouette', [1.2, 0.05, 0.36], [x, 1.18, -14.2], shadow);
    watchPlatform.rotation.y = sideIndex ? -0.08 : 0.08;
    box('north-star shell tiny observation platform rim light', [0.92, 0.032, 0.04], [x, 1.32, -14.0], sideIndex ? cyan : amber);
    box('north-star shell tiny observation platform backstop', [0.08, 0.46, 0.055], [x + (sideIndex ? -0.52 : 0.52), 1.48, -14.1], shadow);
    box('north-star shell tiny observation platform far rail', [0.72, 0.03, 0.04], [x, 1.62, -14.32], sideIndex ? cyan : amber);
  });
}


function buildExteriorStationInfrastructure() {
  const voidMat = mat(0x000105, { roughness: 1.0, metalness: 0.0 });
  const graphite = mat(0x111a28, { roughness: 0.52, metalness: 0.66 });
  const darkSteel = mat(0x060a12, { roughness: 0.62, metalness: 0.58 });
  const railSteel = mat(0x26364c, { roughness: 0.44, metalness: 0.72 });
  const coldGlass = mat(0x061526, { roughness: 0.18, metalness: 0.28, transparent: true, opacity: 0.34, emissive: 0x081d35, emissiveIntensity: 0.12 });
  const cyanDim = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.2, transparent: true, opacity: 0.15 });
  const amberDim = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.18, transparent: true, opacity: 0.14 });
  const greenDim = mat(COLORS.green, { emissive: COLORS.green, emissiveIntensity: 0.16, transparent: true, opacity: 0.13 });
  const violetDim = mat(COLORS.violet, { emissive: COLORS.violet, emissiveIntensity: 0.16, transparent: true, opacity: 0.13 });

  // North-star gap: prove this room belongs to a larger working asteroid base, not only a nice interior aperture.
  // These exterior decks, tubes, antennae, and dock arms sit outside/behind the command insert and read at overview distance.
  const exteriorDecks = [
    ['upper left observation gallery beyond glass', -12.8, 5.55, -20.6, 9.2, COLORS.violet, -0.04],
    ['upper right observation gallery beyond glass', 12.8, 5.55, -20.6, 9.2, COLORS.green, 0.04],
    ['mid left cargo transfer deck beyond glass', -14.2, 3.55, -24.8, 7.4, COLORS.gold, -0.055],
    ['mid right launch transfer deck beyond glass', 14.2, 3.55, -24.8, 7.4, COLORS.coral, 0.055],
    ['deep center command support deck beyond glass', 0, 4.4, -31.2, 10.8, COLORS.cyan, 0]
  ];

  exteriorDecks.forEach(([name, x, y, z, width, color, rz], index) => {
    const group = new THREE.Group();
    ...
    const accent = mat(color, { emissive: color, emissiveIntensity: 0.2, transparent: true, opacity: 0.16 });
    box('exterior station deck black void behind pressure glass', [width + 1.4, 2.25 + index * 0.12, 0.1], [0, y, -0.18], voidMat, group);
    box('exterior station deck suspended slab', [width, 0.08, 0.54], [0, y - 0.86, 0.16], index % 2 ? graphite : darkSteel, group);
    box('exterior station deck upper truss', [width * 0.92, 0.065, 0.06], [0, y + 0.94, 0.04], railSteel, group);
    box('exterior station deck glowing handrail', [width * 0.74, 0.04, 0.045], [0, y - 0.55, 0.42], accent, group);
    box('exterior station deck cold glass safety wall', [width * 0.62, 0.84, 0.045], [0, y - 0.08, 0.48], coldGlass, group);

    [-1, 1].forEach((side) => {
      box('exterior station deck pressure side post', [0.12, 1.9, 0.08], [side * width * 0.48, y, 0.16], railSteel, group);
      const brace = box('exterior station deck diagonal pressure brace', [0.055, 1.45, 0.055], [side * width * 0.33, y + 0.02, 0.22], darkSteel, group);
      brace.rotation.z = side * (0.28 + index * 0.02);
    });

    for (let i = 0; i < 6; i += 1) {
      const px = -width * 0.38 + i * (width * 0.76 / 5);
      box('exterior station deck tiny module window', [0.16, 0.05, 0.035], [px, y + 0.36 + (i % 3) * 0.16, 0.52], i % 2 ? accent : cyanDim, group);
      box('exterior station deck repeating railing upright', [0.045, 0.54, 0.04], [px, y - 0.28, 0.45], i % 2 ? railSteel : darkSteel, group);
    }
  });

  const dockArms = [
    [-18.2, -15.8, -8.6, -24.4, COLORS.violet],
    [18.2, -15.8, 8.6, -24.4, COLORS.green],
    [-16.8, -3.2, -10.4, -20.2, COLORS.gold],
    [16.8, -3.2, 10.4, -20.2, COLORS.coral]
  ];

  dockArms.forEach(([sx, sz, ex, ez, color], index) => {
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(sx, 2.35, sz),
      new THREE.Vector3((sx + ex) * 0.52, 2.8 + index * 0.12, (sz + ez) * 0.5),
      new THREE.Vector3(ex, 3.2 + index * 0.08, ez)
    ]);
    const tube = new THREE.Mesh(new THREE.TubeGeometry(curve, 38, 0.05, 8, false), index % 2 ? graphite : darkSteel);
    tube.name = 'north-star exterior pressurized transit tube';
    root.add(tube);

    const glow = new THREE.Mesh(new THREE.TubeGeometry(curve, 38, 0.018, 8, false), mat(color, { emissive: color, emissiveIntensity: 0.2, transparent: true, opacity: 0.16 }));
    glow.name = 'north-star exterior transit tube running light';
    root.add(glow);

    for (let i = 0; i < 5; i += 1) {
      const p = curve.getPoint((i + 1) / 6);
      const ring = box('exterior transit tube clamp ring', [0.42, 0.055, 0.055], [p.x, p.y, p.z], i % 2 ? railSteel : darkSteel);
      ring.rotation.y = index % 2 ? -0.22 : 0.22;
    }
  });

  const antennaClusters = [
    [-21.8, 6.7, -18.8, -0.14],
    [21.8, 6.7, -18.8, 0.14],
    [-10.2, 6.25, -29.0, -0.08],
    [10.2, 6.25, -29.0, 0.08]
  ...
  antennaClusters.forEach(([x, y, z, rz], index) => {
    const mast = box('north-star exterior antenna mast silhouette', [0.08, 1.7 + (index % 2) * 0.32, 0.08], [x, y, z], darkSteel);
    mast.rotation.z = rz;
    box('north-star exterior antenna crossbar', [0.72, 0.04, 0.04], [x, y + 0.62, z], index % 2 ? railSteel : graphite).rotation.z = rz * 0.7;
    box('north-star exterior antenna blinking beacon', [0.09, 0.05, 0.05], [x, y + 1.0 + (index % 2) * 0.18, z], index % 2 ? amberDim : cyanDim);
  });
}

function buildLowerMechanicalCutaway() {
  const voidMat = mat(0x000106, { roughness: 1.0, metalness: 0.0 });
  const floorDark = mat(0x070d16, { roughness: 0.64, metalness: 0.44 });
  const graphite = mat(0x121a28, { roughness: 0.5, metalness: 0.62 });
  const steel = mat(0x26364d, { roughness: 0.44, metalness: 0.68 });
  const cyanDim = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.18, transparent: true, opacity: 0.14 });
  const amberDim = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.16, transparent: true, opacity: 0.13 });
  const shadowGlass = mat(0x020711, { roughness: 0.16, metalness: 0.25, transparent: true, opacity: 0.44, emissive: 0x061526, emissiveIntensity: 0.08 });

  // Vertical-slice proof: show a lower mechanical level under the command deck so the wide view reads as a cut through a larger base.
  const lowerBays = [
    [-12.4, -0.62, 4.3, 6.8, COLORS.gold],
    [0, -0.78, 4.1, 8.2, COLORS.cyan],
    [12.4, -0.62, 4.3, 6.8, COLORS.coral],
    [-10.8, -0.9, -6.4, 6.0, COLORS.violet],
    [10.8, -0.9, -6.4, 6.0, COLORS.green]
  ];

  lowerBays.forEach(([x, y, z, width, color], index) => {
    const accent = mat(color, { emissive: color, emissiveIntensity: 0.18, transparent: true, opacity: 0.14 });
    const group = new THREE.Group();
    group.name = 'north-star lower mechanical exposed cutaway bay';
    group.position.set(x, 0, z);
    root.add(group);

    box('lower mechanical black underfloor negative space', [width, 1.15, 1.25], [0, y, 0], voidMat, group);
    box('lower mechanical top deck sliced edge', [width * 0.92, 0.075, 0.12], [0, y + 0.62, 0.56], index % 2 ? graphite : steel, group);
    box('lower mechanical bottom service deck', [width * 0.84, 0.08, 0.56], [0, y - 0.48, -0.05], floorDark, group);
    box('lower mechanical glass inspection strip', [width * 0.66, 0.48, 0.045], [0, y + 0.08, 0.62], shadowGlass, group);
    box('lower mechanical accent service rail', [width * 0.64, 0.035, 0.045], [0, y + 0.36, 0.68], accent, group);

    [-1, 1].forEach((side) => {
      box('lower mechanical vertical service support', [0.08, 1.05, 0.08], [side * width * 0.43, y + 0.02, 0.08], steel, group);
      const brace = box('lower mechanical diagonal cutaway brace', [0.055, 0.88, 0.055], [side * width * 0.26, y - 0.02, 0.1], graphite, group);
      brace.rotation.z = side * 0.28;
    });

    for (let i = 0; i < 5; i += 1) {
      const px = -width * 0.34 + i * (width * 0.17);
      box('lower mechanical tiny machinery status light', [0.11, 0.035, 0.032], [px, y + 0.2 + (i % 2) * 0.18, 0.7], i % 2 ? cyanDim : amberDim, group);
      box('lower mechanical repeating pipe silhouette', [0.045, 0.48, 0.04], [px + 0.26, y - 0.05, 0.32], i % 2 ? graphite : steel, group);
    }
  });

  for (let i = 0; i < 8; i += 1) {
    const x = -15.8 + i * 4.5;
    const shaft = box('lower mechanical vertical utility shaft descending from command deck', [0.16, 1.35 + (i % 2) * 0.25, 0.16], [x, -0.18, -1.8 - (i % 3) * 2.1], i % 2 ? graphite : steel);
    shaft.rotation.z = -0.035 + i * 0.01;
    box('lower mechanical shaft dim marker', [0.12, 0.045, 0.04], [x, 0.48, -1.65 - (i % 3) * 2.1], i % 2 ? cyanDim : amberDim);
  }
}

function buildViewportBlastShutterHierarchy() {
  const shutter = mat(0x0b111d, { roughness: 0.5, metalness: 0.7 });
  const hinge = mat(0x27374f, { roughness: 0.42, metalness: 0.76 });
  const blackGlass = mat(0x020611, { roughness: 0.14, metalness: 0.3, transparent: true, opacity: 0.48, emissive: 0x061a2d, emissiveIntensity: 0.08 });
  const coldEdge = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.22, transparent: true, opacity: 0.16 });
  const warmEdge = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.2, transparent: true, opacity: 0.15 });
  const shadow = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.64 });

  // Large segmented shutters add pressure-rated shell hierarchy and reduce equal neon noise around the glass.
  const shutters = [
    [-15.6, 6.55, -16.5, 7.2, -0.09],
    [-6.2, 6.82, -16.8, 6.4, 0.035],
    [6.2, 6.82, -16.8, 6.4, -0.035],
    [15.6, 6.55, -16.5, 7.2, 0.09],
    [-17.8, 2.2, -17.2, 5.5, 0.08],
    [17.8, 2.2, -17.2, 5.5, -0.08]
  ];

  shutters.forEach(([x, y, z, width, rz], index) => {
    const panel = box('north-star segmented viewport blast shutter plate', [width, 0.22, 0.18], [x, y, z], index % 2 ? shutter : hinge);
    panel.rotation.z = rz;
    panel.rotation.y = -0.04 + index * 0.016;
    box('north-star viewport shutter black separation groove', [width * 0.72, 0.052, 0.045], [x, y - 0.22, z + 0.12], shadow).rotation.y = panel.rotation.y;
    box('north-star viewport shutter small edge light', [width * 0.52, 0.035, 0.04], [x, y + 0.14, z + 0.15], index % 2 ? coldEdge : warmEdge).rotation.y = panel.rotation.y;
  });

  const verticalPanels = [
    [-18.8, 4.5, -18.2, 3.1, -0.05],
    [-12.2, 4.15, -19.2, 2.6, 0.04],
    [12.2, 4.15, -19.2, 2.6, -0.04],
    [18.8, 4.5, -18.2, 3.1, 0.05]
  ];

  verticalPanels.forEach(([x, y, z, height, rz], index) => {
    const panel = box('north-star vertical viewport pressure shutter jamb', [0.18, height, 0.16], [x, y, z], index % 2 ? hinge : shutter);
    panel.rotation.z = rz;
    box('north-star vertical shutter inner glass reveal', [0.055, height * 0.72, 0.045], [x * 0.99, y, z + 0.12], blackGlass).rotation.z = rz * 0.8;
    box('north-star vertical shutter tiny lock indicator', [0.05, 0.18, 0.035], [x * 0.99, y + height * 0.28, z + 0.16], index % 2 ? coldEdge : warmEdge);
  });

  for (let i = 0; i < 12; i += 1) {
    const x = -16.5 + i * 3;
    const latch = box('north-star viewport shutter latch tooth repeating scale', [0.32, 0.06, 0.08], [x, 6.22 + (i % 2) * 0.12, -15.9], i % 2 ? hinge : shutter);
    latch.rotation.y = -0.1 + i * 0.018;
  }
}


function buildExternalDockedSilhouetteNetwork() {
  const hull = mat(0x090d16, { roughness: 0.58, metalness: 0.62 });
  const hullLight = mat(0x1b293b, { roughness: 0.46, metalness: 0.7 });
  const shadow = mat(0x000105, { roughness: 1.0, metalness: 0.0 });
  const cyan = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.18, transparent: true, opacity: 0.15 });
  const amber = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.16, transparent: true, opacity: 0.14 });
  const green = mat(COLORS.green, { emissive: COLORS.green, emissiveIntensity: 0.14, transparent: true, opacity: 0.12 });

  // Exterior station visibility: docked craft and utility frames show the base doing work beyond the command window.
  const dockedCraft = [
    [-16.4, 3.1, -28.8, 1.25, COLORS.violet, -0.18],
    [16.4, 3.05, -29.4, 1.18, COLORS.green, 0.18],
    [-7.2, 4.1, -35.8, 0.92, COLORS.gold, -0.1],
    [7.2, 4.0, -36.4, 0.9, COLORS.coral, 0.1],
    [0, 4.7, -43.2, 0.68, COLORS.cyan, 0]
  ];

  dockedCraft.forEach(([x, y, z, scale, color, ry], index) => {
    const accent = mat(color, { emissive: color, emissiveIntensity: 0.18, transparent: true, opacity: 0.14 });
    const ship = new THREE.Group();
    ship.name = 'north-star exterior docked operations craft silhouette';
    ship.position.set(x, y, z);
    ship.rotation.y = ry;
    ship.scale.setScalar(scale);
    root.add(ship);

    box('docked operations craft central dark hull', [1.8, 0.14, 0.22], [0, 0, 0], hull, ship);
    box('docked operations craft armored nose', [0.34, 0.18, 0.24], [1.0, 0.02, 0], hullLight, ship);
    box('docked operations craft rear drive block', [0.38, 0.16, 0.3], [-0.96, 0.01, 0], shadow, ship);
    box('docked operations craft left outrigger', [0.72, 0.06, 0.5], [-0.18, -0.02, 0.44], index % 2 ? hull : hullLight, ship);
    box('docked operations craft right outrigger', [0.72, 0.06, 0.5], [-0.18, -0.02, -0.44], index % 2 ? hull : hullLight, ship);
    box('docked operations craft running light nose', [0.08, 0.04, 0.04], [1.18, 0.06, 0], accent, ship);
    box('docked operations craft belly work light', [0.16, 0.035, 0.035], [-0.2, -0.08, 0], index % 2 ? cyan : amber, ship);

    const dock = new THREE.Group();
    dock.name = 'north-star exterior craft docking cradle';
    dock.position.set(x, y - 0.36, z + 0.42);
    dock.rotation.y = ry;
    dock.scale.setScalar(scale);
    root.add(dock);
    box('craft docking cradle spine', [2.2, 0.065, 0.08], [0, 0, 0], hullLight, dock);
    box('craft docking cradle left clamp', [0.1, 0.32, 0.08], [-0.74, 0.12, 0.28], hull, dock);
    box('craft docking cradle right clamp', [0.1, 0.32, 0.08], [0.74, 0.12, 0.28], hull, dock);
    box('craft docking cradle status lamp', [0.12, 0.04, 0.04], [0, 0.16, 0.08], accent, dock);
  });

  const utilityFrames = [
    [-20.5, 2.6, -23.5, 3.6, COLORS.violet],
    [20.5, 2.6, -23.5, 3.6, COLORS.green],
    [-12.0, 5.1, -33.5, 2.8, COLORS.gold],
    [12.0, 5.1, -33.5, 2.8, COLORS.coral]
  ];

  utilityFrames.forEach(([x, y, z, height, color], index) => {
    const accent = mat(color, { emissive: color, emissiveIntensity: 0.15, transparent: true, opacity: 0.12 });
    box('exterior utility frame vertical megastructure post', [0.14, height, 0.11], [x, y, z], index % 2 ? hull : hullLight);
    box('exterior utility frame crossmember', [2.0, 0.07, 0.08], [x, y + height * 0.42, z], hullLight).rotation.z = index % 2 ? 0.08 : -0.08;
    box('exterior utility frame lower crossmember', [1.6, 0.055, 0.06], [x, y - height * 0.35, z + 0.08], hull).rotation.z = index % 2 ? -0.05 : 0.05;
    box('exterior utility frame scale beacon', [0.1, 0.05, 0.04], [x, y + height * 0.55, z + 0.1], accent);

    for (let i = 0; i < 3; i += 1) {
      const rung = box('exterior utility frame ladder rung scale cue', [0.58, 0.035, 0.035], [x, y - height * 0.2 + i * 0.34, z + 0.12], i % 2 ? hullLight : hull);
      rung.rotation.z = index % 2 ? 0.04 : -0.04;
    }
  });

  const cargoTracks = [
    [-18.0, -24.0, -5.0, -31.0, COLORS.gold],
    [18.0, -24.0, 5.0, -31.0, COLORS.green],
    [-8.5, -30.0, 0, -38.5, COLORS.cyan],
    [8.5, -30.0, 0, -38.5, COLORS.cyan]
  ];

  cargoTracks.forEach(([x1, z1, x2, z2, color], index) => {
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(x1, 1.55, z1),
      new THREE.Vector3((x1 + x2) * 0.5, 1.92 + index * 0.08, (z1 + z2) * 0.5),
      new THREE.Vector3(x2, 2.4 + index * 0.06, z2)
    ]);
    const railA = new THREE.Mesh(new THREE.TubeGeometry(curve, 32, 0.018, 8, false), hullLight);
    railA.name = 'north-star exterior cargo track rail silhouette';
    root.add(railA);
    const railGlow = new THREE.Mesh(new THREE.TubeGeometry(curve, 32, 0.01, 8, false), mat(color, { emissive: color, emissiveIntensity: 0.14, transparent: true, opacity: 0.12 }));
    railGlow.name = 'north-star exterior cargo track low guide light';
    root.add(railGlow);
  });
}

function buildMassiveHullLoadRibs() {
  const rib = mat(0x0b111d, { roughness: 0.48, metalness: 0.72 });
  const ribFace = mat(0x1d2b3d, { roughness: 0.44, metalness: 0.78 });
  const black = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.72 });
  const cyan = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.18, transparent: true, opacity: 0.14 });
  const amber = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.16, transparent: true, opacity: 0.13 });

  // Heavy load ribs make the asteroid shell feel pressure-rated and massive at screenshot scale.
  const ribs = [
    [-24.8, 3.5, 5.6, 7.8, -0.11],
    [-25.4, 3.7, -3.6, 8.4, 0.06],
    [-23.6, 4.5, -12.2, 6.2, -0.04],
    [24.8, 3.5, 5.6, 7.8, 0.11],
    [25.4, 3.7, -3.6, 8.4, -0.06],
    [23.6, 4.5, -12.2, 6.2, 0.04]
  ];

  ribs.forEach(([x, y, z, height, rz], index) => {
    const main = box('north-star massive hull load rib outside asteroid cut', [0.28, height, 0.24], [x, y, z], index % 2 ? rib : ribFace);
    main.rotation.z = rz;
    main.rotation.y = x < 0 ? 0.08 : -0.08;
    const innerShadow = box('north-star massive hull rib black contact shadow', [0.08, height * 0.78, 0.08], [x * 0.985, y, z - 0.36], black);
    innerShadow.rotation.z = rz * 0.7;
    innerShadow.rotation.y = main.rotation.y;

    for (let i = 0; i < 5; i += 1) {
      const yy = y - height * 0.36 + i * (height * 0.18);
      const strap = box('north-star hull load rib transverse clamp strap', [0.34, 0.05, 0.42], [x, yy, z + 0.1], i % 2 ? ribFace : rib);
      strap.rotation.z = rz;
      strap.rotation.y = main.rotation.y;
      if (i === 1 || i === 3) {
        box('north-star hull load rib small inspection light', [0.06, 0.04, 0.04], [x * 0.99, yy + 0.06, z + 0.34], i % 2 ? cyan : amber);
      }
    }
  });

  const crownRibs = [
    [-20.4, 8.92, 3.2, 6.4, -0.05],
    [-11.8, 9.18, -1.0, 7.8, 0.04],
    [0, 9.28, -4.8, 8.8, 0],
    [11.8, 9.18, -1.0, 7.8, -0.04],
    [20.4, 8.92, 3.2, 6.4, 0.05]
  ];

  crownRibs.forEach(([x, y, z, width, rz], index) => {
    const cap = box('north-star overhead hull load rib segment', [width, 0.26, 0.24], [x, y, z], index % 2 ? rib : ribFace);
    cap.rotation.z = rz;
    cap.rotation.y = -0.06 + index * 0.03;
    box('north-star overhead hull rib underside occlusion', [width * 0.72, 0.07, 0.08], [x, y - 0.32, z + 0.22], black).rotation.y = cap.rotation.y;
    box('north-star overhead hull rib sparse service light', [width * 0.42, 0.035, 0.04], [x, y - 0.18, z - 0.18], index % 2 ? cyan : amber).rotation.y = cap.rotation.y;
  });

  for (let i = 0; i < 16; i += 1) {
    const x = -22.0 + i * 2.95;
    const z = 9.5 + Math.sin(i * 0.6) * 0.22;
    const sillRib = box('north-star lower sill load-rib tooth', [1.2 + (i % 3) * 0.22, 0.12, 0.18], [x, 0.82 + (i % 2) * 0.04, z], i % 2 ? ribFace : rib);
    sillRib.rotation.y = -0.18 + (i % 6) * 0.07;
  }
}
...
function buildBaseNetworkSignalArcs() {
  ...
  const amber = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.15, transparent: true, opacity: 0.11 });
  const violet = mat(COLORS.violet, { emissive: COLORS.violet, emissiveIntensity: 0.14, transparent: true, opacity: 0.1 });
  const green = mat(COLORS.green, { emissive: COLORS.green, emissiveIntensity: 0.14, transparent: true, opacity: 0.1 });
  const dark = mat(0x060b13, { roughness: 0.66, metalness: 0.5 });

  // Tertiary network lines show Mission Control coordinating an exterior base network, not floating alone.
  const arcs = [
    [-12.8, -10.65, -18.2, -24.8, violet],
    [12.8, -10.65, 18.2, -24.8, green],
    [-12.75, -0.25, -18.0, -18.0, amber],
    [12.75, -0.45, 18.0, -18.0, cyan],
    [0, 0.55, 0, -31.0, cyan]
  ];

  arcs.forEach(([x1, z1, x2, z2, material], index) => {
    const curve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(x1, 1.35, z1),
      new THREE.Vector3((x1 + x2) * 0.5, 2.2 + index * 0.08, (z1 + z2) * 0.48),
      new THREE.Vector3(x2, 2.95 + index * 0.06, z2)
    ]);
    const tube = new THREE.Mesh(new THREE.TubeGeometry(curve, 36, 0.014, 8, false), material);
    tube.name = 'north-star exterior base network signal arc';
    root.add(tube);

    for (let i = 0; i < 3; i += 1) {
      const p = curve.getPoint((i + 1) / 4);
      const relay = box('north-star exterior network relay node', [0.1, 0.055, 0.08], [p.x, p.y, p.z], i % 2 ? material : dark);
      relay.rotation.y = i * 0.4;
    }
  });

  const relayTowers = [
    [-20.6, 4.2, -26.0, 1.2, violet],
    [20.6, 4.2, -26.0, 1.2, green],
    [-5.0, 5.0, -34.5, 0.9, amber],
    [5.0, 5.0, -34.5, 0.9, cyan]
  ];

  relayTowers.forEach(([x, y, z, height, material], index) => {
    const mast = box('north-star exterior network relay tower silhouette', [0.07, height, 0.07], [x, y, z], dark);
    mast.rotation.z = x < 0 ? -0.04 : 0.04;
    box('north-star exterior network relay tower beacon', [0.1, 0.05, 0.05], [x, y + height * 0.54, z], material);
    box('north-star exterior network relay tower crossbar', [0.54, 0.035, 0.035], [x, y + height * 0.25, z], dark).rotation.z = index % 2 ? 0.08 : -0.08;
    box('north-star exterior network relay base foot', [0.22, 0.045, 0.14], [x, y - height * 0.52, z], dark);
    box('north-star exterior network relay grounding light', [0.08, 0.035, 0.035], [x, y - height * 0.44, z + 0.08], material);
  });
}

function buildCommandLightingHierarchy() {
  const commandBlue = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.34, transparent: true, opacity: 0.12, side: THREE.DoubleSide });
  const dimWarm = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.16, transparent: true, opacity: 0.035, side: THREE.DoubleSide });
  const dimCool = mat(0x7dbbff, { emissive: 0x7dbbff, emissiveIntensity: 0.14, transparent: true, opacity: 0.03, side: THREE.DoubleSide });
  const mask = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.24, side: THREE.DoubleSide });
  const cyanDim = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.18, transparent: true, opacity: 0.13 });

  // Lighting hierarchy pass: make command glow primary, stations secondary, exterior tertiary.
  const commandAura = new THREE.Mesh(new THREE.PlaneGeometry(9.2, 6.4), commandBlue);
  commandAura.name = 'north-star primary command glow hierarchy plane';
  commandAura.position.set(0, 1.62, -0.4);
  commandAura.rotation.x = -0.42;
  root.add(commandAura);
  animated.push((t) => { commandAura.material.opacity = 0.1 + Math.sin(t * 0.38) * 0.012; });

  const tertiaryMasks = [
    [-13.6, 2.8, 2.0, 7.2, 2.1],
    [13.6, 2.8, 2.0, 7.2, 2.1],
    [-13.4, 2.9, -9.8, 7.0, 2.2],
    [13.4, 2.9, -9.8, 7.0, 2.2]
  ];

  tertiaryMasks.forEach(([x, y, z, w, h], index) => {
    const panel = new THREE.Mesh(new THREE.PlaneGeometry(w, h), index % 2 ? dimWarm : dimCool);
    panel.name = 'north-star secondary district controlled light pool';
    panel.position.set(x, y, z);
    panel.rotation.x = -0.2;
    panel.rotation.z = x < 0 ? -0.04 : 0.04;
    root.add(panel);
    animated.push((t) => { panel.material.opacity = (index % 2 ? 0.03 : 0.026) + Math.sin(t * 0.3 + index) * 0.004; });

    const shadow = new THREE.Mesh(new THREE.PlaneGeometry(w * 1.1, h * 0.7), mask);
    shadow.name = 'north-star district light hierarchy shadow matte';
    shadow.position.set(x, 0.09, z + 0.9);
    shadow.rotation.x = -Math.PI / 2;
    root.add(shadow);
  });

  addLight('point', COLORS.cyan, 1.6, [0, 2.0, 0.35], 11.5);
  addLight('point', 0x7dbbff, 1.0, [0, 4.2, -19.0], 24.0);
  addLight('point', COLORS.amber, 0.75, [-18.5, 6.2, 4.8], 18.0);
  addLight('point', COLORS.amber, 0.75, [18.5, 6.2, 4.8], 18.0);

  for (let i = 0; i < 18; i += 1) {
    const angle = (i / 18) * Math.PI * 2;
    const x = Math.cos(angle) * 5.8;
    const z = 0.55 + Math.sin(angle) * 3.8;
    const tick = box('north-star command hierarchy subtle floor tick', [0.18, 0.026, 0.045], [x, 0.115, z], cyanDim);
    tick.rotation.y = -angle;
  }
}

function buildExteriorColdVistaScale() {
  const voidMat = mat(0x000106, { roughness: 1.0, metalness: 0.0 });
  const coldGlass = mat(0x061525, { roughness: 0.16, metalness: 0.28, transparent: true, opacity: 0.4, emissive: 0x0a2440, emissiveIntensity: 0.16 });
  const steel = mat(0x182233, { roughness: 0.48, metalness: 0.66 });
  const darkSteel = mat(0x070b12, { roughness: 0.58, metalness: 0.55 });
  const cyanDim = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.18, transparent: true, opacity: 0.14 });
  const amberDim = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.16, transparent: true, opacity: 0.13 });

  // Cold exterior vistas behind the operation floor add the Star Citizen / Star Atlas scale layer without adding interior prop noise.
  const panes = [
    [0, 3.55, -18.9, 30.5, 4.9, 0],
    [0, 4.0, -25.6, 23.0, 4.1, 0.02],
    [0, 4.32, -33.0, 15.5, 3.25, -0.015]
  ];

  panes.forEach(([x, y, z, w, h, ry], i) => {
    const glass = box('north-star cold exterior pressure glass layer', [w, h, 0.055], [x, y, z], i === 0 ? coldGlass : voidMat);
    glass.rotation.y = ry;
    box('north-star cold exterior upper window truss', [w * 0.82, 0.075, 0.07], [x, y + h * 0.46, z + 0.12], i % 2 ? steel : darkSteel);
    box('north-star cold exterior lower window truss', [w * 0.7, 0.06, 0.065], [x, y - h * 0.44, z + 0.12], darkSteel);
    [-1, 1].forEach((side) => {
      box('north-star cold exterior side window truss', [0.09, h * 0.82, 0.06], [side * w * 0.42, y, z + 0.14], steel);
      const diagonal = box('north-star cold exterior diagonal hangar rib', [0.055, h * 0.6, 0.055], [side * w * 0.26, y, z + 0.16], darkSteel);
      diagonal.rotation.z = side * (0.22 + i * 0.04);
    });
  });

  const asteroidSilhouettes = [
    [-10.8, 5.8, -37.0, 1.35, 0.8],
    [11.2, 4.9, -35.6, 1.05, 1.1],
    [-4.6, 6.2, -42.5, 0.72, 0.6],
    [5.0, 5.7, -43.2, 0.86, 0.75],
    [0, 3.4, -39.4, 1.12, 0.9]
  ];

  asteroidSilhouettes.forEach(([x, y, z, scale, stretch], i) => {
    const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(scale, 0), mat(0x06070c, { roughness: 0.98, metalness: 0.0 }));
    rock.name = 'north-star exterior asteroid silhouette beyond glass';
    rock.position.set(x, y, z);
    rock.rotation.set(i * 0.45, i * 0.32, i * 0.2);
    rock.scale.set(1.5, 0.74 + stretch * 0.2, 1.0 + stretch * 0.25);
  ...
  const shipProfiles = [
    [-7.4, 2.9, -29.8, 1.0, cyanDim],
    [7.0, 2.75, -30.5, 0.82, amberDim],
    [0.6, 3.55, -36.4, 0.68, cyanDim]
  ];

  shipProfiles.forEach(([x, y, z, scale, lightMat], i) => {
    const ship = new THREE.Group();
    ship.name = 'north-star exterior docked ship scale silhouette';
    ship.position.set(x, y, z);
    ship.scale.setScalar(scale);
    ship.rotation.y = -0.12 + i * 0.16;
    root.add(ship);
    box('north-star exterior ship central spine', [1.55, 0.1, 0.18], [0, 0, 0], darkSteel, ship);
    box('north-star exterior ship bridge block', [0.32, 0.18, 0.22], [0.32, 0.12, 0], steel, ship);
    box('north-star exterior ship left foil', [0.58, 0.055, 0.48], [-0.28, -0.02, 0.38], steel, ship);
    box('north-star exterior ship right foil', [0.58, 0.055, 0.48], [-0.28, -0.02, -0.38], steel, ship);
    box('north-star exterior ship nose light', [0.08, 0.04, 0.04], [0.86, 0.02, 0], lightMat, ship);
  });

  for (let row = 0; row < 4; row += 1) {
    const z = -22.2 - row * 4.6;
    const width = 27.0 - row * 4.0;
    for (let i = 0; i < 9 - row; i += 1) {
      const x = -width * 0.42 + i * (width * 0.84 / Math.max(1, 8 - row));
      const y = 2.2 + row * 0.5 + (i % 3) * 0.25;
      box('north-star exterior hangar pinlight row through glass', [0.1, 0.035, 0.03], [x, y, z], (i + row) % 2 ? cyanDim : amberDim);
    }
  }
}

function buildCinematicShellContrastPass() {
  const cool = mat(0x72b7ff, { emissive: 0x72b7ff, emissiveIntensity: 0.18, transparent: true, opacity: 0.028, side: THREE.DoubleSide });
  const warm = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.12, transparent: true, opacity: 0.018, side: THREE.DoubleSide });
  const black = mat(0x000105, { roughness: 1.0, metalness: 0.0, transparent: true, opacity: 0.32, side: THREE.DoubleSide });
  const edgeCyan = mat(COLORS.cyan, { emissive: COLORS.cyan, emissiveIntensity: 0.24, transparent: true, opacity: 0.16 });
  const edgeAmber = mat(COLORS.amber, { emissive: COLORS.amber, emissiveIntensity: 0.22, transparent: true, opacity: 0.14 });

  // Lighting rebuild: separate dark shell, readable station floor, and cold exterior beyond instead of washing the whole scene flat.
  const rearKey = addLight('directional', 0x8fc2ff, 0.36, [0, 10.5, -38.0]);
  rearKey.name = 'north-star cold exterior silhouette key';
  rearKey.target.position.set(0, 2.2, -4.5);
  scene.add(rearKey.target);

  const warmPractical = addLight('directional', 0xffc76d, 0.18, [0, 8.6, 10.8]);
  warmPractical.name = 'north-star warm interior practical graze';
  warmPractical.target.position.set(0, 1.0, -1.2);
  scene.add(warmPractical.target);

  addLight('point', COLORS.cyan, 1.25, [0, 2.4, 0.3], 12.0);
  addLight('point', COLORS.amber, 1.1, [-18.4, 6.2, 5.2], 18.0);
  addLight('point', COLORS.amber, 1.1, [18.4, 6.2, 5.2], 18.0);

  const veils = [
    [0, 4.05, -18.2, 31.0, 4.6, cool, -0.035],
    [0, 4.35, -27.4, 22.0, 3.8, cool, -0.02],
    [0, 2.0, 6.6, 34.0, 1.7, warm, -0.18],
    [-13.6, 2.8, 1.8, 7.8, 2.5, warm, -0.12],
    [13.6, 2.8, 1.8, 7.8, 2.5, warm, 0.12]
  ];

  veils.forEach(([x, y, z, w, h, material, rx], i) => {
    const veil = new THREE.Mesh(new THREE.PlaneGeometry(w, h), material.clone());
    veil.name = 'north-star cinematic layered light veil';
    veil.position.set(x, y, z);
    veil.rotation.x = rx;
    veil.rotation.z = i % 2 ? 0.018 : -0.018;
    root.add(veil);
    const baseOpacity = veil.material.opacity;
    animated.push((t) => { veil.material.opacity = baseOpacity + Math.sin(t * 0.2 + i) * 0.004; });
  });

  const shellShadows = [
    [-23.7, 3.0, 4.8, 0.4, 6.8, 11.0],
    [23.7, 3.0, 4.8, 0.4, 6.8, 11.0],
    [-22.8, 4.3, -8.4, 0.32, 5.4, 8.8],
    [22.8, 4.3, -8.4, 0.32, 5.4, 8.8],
    [0, 8.6, 1.4, 34.0, 0.42, 8.0],
    [0, 0.18, 9.8, 36.0, 0.32, 1.2]
  ];

  shellShadows.forEach(([x, y, z, w, h, d], i) => {
    const shadow = box('north-star shell contrast black occlusion band', [w, h, d], [x, y, z], black);
    shadow.rotation.z = x < 0 ? -0.035 : x > 0 ? 0.035 : 0;
    shadow.renderOrder = 3;
  });

  const rimLines = [
    [-22.8, 2.6, 6.8, 0.045, 4.8, edgeCyan, -0.05],
    [22.8, 2.6, 6.8, 0.045, 4.8, edgeCyan, 0.05],
    [-22.2, 4.4, -5.4, 0.045, 5.2, edgeAmber, -0.035],
    [22.2, 4.4, -5.4, 0.045, 5.2, edgeAmber, 0.035],
    [-13.6, 7.74, 5.4, 8.0, 0.04, edgeCyan, -0.02],
    [13.6, 7.74, 5.4, 8.0, 0.04, edgeCyan, 0.02],
    [0, 7.44, -6.4, 18.0, 0.04, edgeAmber, 0]
  ];

  rimLines.forEach(([x, y, z, w, h, material, rz]) => {
    const size = w < 0.1 ? [0.055, h, 0.05] : [w, h, 0.05];
    const rim = box('north-star cinematic shell rim catchlight', size, [x, y, z], material);
    rim.rotation.z = rz;
  });
}
